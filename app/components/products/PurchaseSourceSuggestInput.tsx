...
type PurchaseSourceSuggestInputProps = {
  id: string;
  name?: string;
  value: string;
...
export function PurchaseSourceSuggestInput({
  id,
  name,
  value,
  onChange,
...
}: PurchaseSourceSuggestInputProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const snap = await getDocs(collection(getDb(), COLLECTIONS.stockLots));
        const set = new Set<string>();
        snap.forEach((docSnap) => {
          const raw = (docSnap.data() as StockLotDoc).purchase_source;
